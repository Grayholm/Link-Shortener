...
@pytest.mark.parametrize("long_url, should_raise, expected_status_code", [
...
async def test_redirect_to_url_API(ac: AsyncClient, long_url, should_raise, expected_status_code):
    ...
    if should_raise:
        redirect_response = await ac.get(f"/api/short-links/FGGg46")
...
async def test_create_short_link_API(ac: AsyncClient):
    response = await ac.post("/api/short-links", json={"long_url": "https://www.example.com/"})
